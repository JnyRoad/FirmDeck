...
def test_firmdeck_seed_reads_fixture_as_utf8(monkeypatch) -> None:
    ...
    monkeypatch.setattr(firmdeck_seed, "FIXTURE_PATH", FakeFixturePath())
    monkeypatch.setattr(firmdeck_seed, "_seed_agents", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(firmdeck_seed, "_seed_skills", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(firmdeck_seed, "_seed_general_skills", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(firmdeck_seed, "_seed_tools", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(firmdeck_seed, "_seed_knowledge", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(firmdeck_seed, "_seed_agent_resource_bindings", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(firmdeck_seed, "_seed_skill_branches", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(firmdeck_seed, "_seed_knowledge_branches", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(firmdeck_seed, "_publish_gallery_resources", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(firmdeck_seed, "_sync_seed_agents_to_current_admin", lambda *_args, **_kwargs: None)

    firmdeck_seed.seed_firmdeck_admin_gallery(_FlushOnlySession())


def test_firmdeck_seed_requires_every_bundled_fixture(tmp_path) -> None:
    with pytest.raises(FileNotFoundError):
        firmdeck_seed._load_seed_fixtures((tmp_path / "missing-fixture.json",))
...
def test_knowledge_branch_fallback_uses_a_seeded_version_without_documents() -> None:
    ...
    with Session(engine) as db:
        ...
        resolved = firmdeck_seed._knowledge_version_with_seed_content(
...
def test_expanded_firmdeck_skills_match_runtime_schema() -> None:
    data = json.loads(firmdeck_seed.EXPANDED_FIXTURE_PATH.read_text(encoding="utf-8"))
...
def test_firmdeck_seed_exposes_selected_agents_with_knowledge_bases() -> None:
...
def test_firmdeck_seed_adds_expanded_employee_profiles_idempotently() -> None:
    with _seeded_session() as db:
        seed_demo_data(db)
        db.commit()
...
def test_firmdeck_seed_applies_reliability_defaults_to_existing_rows() -> None:
    with _seeded_session() as db:
...
def test_firmdeck_seed_uses_existing_admin_id_for_seeded_agents() -> None:
    engine = create_engine("sqlite:///:memory:")
    SQLModel.metadata.create_all(engine)
    with Session(engine) as db:
...
def test_firmdeck_seed_preserves_custom_avatar_on_restart() -> None:
    with _seeded_session() as db:
        agent = db.exec(
            select(AgentProfile).where(
                AgentProfile.tenant_id == "tenant_demo",
...
def test_firmdeck_seed_does_not_overwrite_non_seed_employee_name_conflict() -> None:
    engine = create_engine("sqlite:///:memory:")
    SQLModel.metadata.create_all(engine)
    with Session(engine) as db:
        db.add(Tenant(id="tenant_demo", name="Demo Enterprise"))
...
def test_firmdeck_seed_archives_legacy_default_agent() -> None:
    engine = create_engine("sqlite:///:memory:")
    SQLModel.metadata.create_all(engine)
    with Session(engine) as db:
        db.add(Tenant(id="tenant_demo", name="Demo Enterprise"))
        ...
        assert row.metadata_json.get("hidden_from_firmdeck") is True
