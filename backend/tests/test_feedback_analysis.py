...
from app.feedback.service import FeedbackAnalysisService, feedback_analysis_read, feedback_summary
...
def test_feedback_analysis_without_model_marks_needs_model() -> None:
    ...
    assert analyzed.analysis_reason == ""
    assert analyzed.analysis_summary == ""


def test_feedback_analysis_failure_keeps_provider_cause_private(monkeypatch) -> None:
    """Persist only stable retry metadata when model failure text contains a private provider cause."""
    def fake_init(self, model_config):
        return None

    def fake_generate_json(self, system_prompt, payload):
        raise LLMError("provider secret: https://private.example.invalid/token")

    monkeypatch.setattr(LLMClient, "__init__", fake_init)
    monkeypatch.setattr(LLMClient, "generate_json", fake_generate_json)
    monkeypatch.setattr(
        "app.feedback.service.FEEDBACK_ANALYSIS_RETRY_DELAY_SECONDS",
        0,
    )
    ...
    with _test_session() as db:
        ...
        assert analyzed is not None
        result = feedback_analysis_read(analyzed)

    assert analyzed.analysis_status == "failed"
    assert analyzed.analysis_bucket == "unknown"
    assert analyzed.analysis_reason == ""
    assert analyzed.analysis_summary == ""
    assert result["status"] == "failed"
    assert result["status_params"] == {"attempts": 3}
