...
SEED_SOURCE = "firmdeck_admin_gallery_seed"
FIXTURE_PATH = Path(__file__).resolve().parent / "seed_fixtures" / "firmdeck_admin_gallery_seed.json"
EXPANDED_FIXTURE_PATH = (
    Path(__file__).resolve().parent / "seed_fixtures" / "firmdeck_expanded_gallery_seed.json"
)
...
_seed_principal: ContextVar[SeedPrincipal] = ContextVar(
    "firmdeck_seed_principal", default=_DEFAULT_SEED_PRINCIPAL
)


def seed_firmdeck_admin_gallery(session: Session) -> None:
    """Seed the curated FirmDeck gallery package as admin-owned resources."""
...
def seed_firmdeck_tenant_gallery(
    session: Session,
    *,
    tenant_id: str,
    admin_user_id: str,
    ...
    try:
        _seed_firmdeck_gallery_for_current_principal(session)
    finally:
        _seed_principal.reset(context_token)


def _seed_firmdeck_gallery_for_current_principal(session: Session) -> None:
...
def _tenant_scoped_fixture_data(data: JsonDict, tenant_id: str) -> JsonDict:
    ...
    id_map = {
        source_id: f"template_{uuid5(NAMESPACE_URL, f'firmdeck:{tenant_id}:{source_id}').hex}"
    ...
    for row in binding_rows:
        ...
        if existing:
            ...
            branch_id = (
                ...
                + uuid5(
                    NAMESPACE_URL,
                    f"firmdeck:{_seed_tenant_id()}:knowledge-branch:{agent_id}:{kb_id}",
                ).hex
            )
