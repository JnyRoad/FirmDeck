...
- 如果生成了需要交付给用户的文件，请写入 `ARTIFACT_DIR`（Python 使用 stdin 的 `artifact_dir`），并在输出 JSON 中声明相对于该目录的路径，例如 `"artifacts":[{"path":"report.xlsx"}]`。不要输出 `/workspace/...` 或宿主机绝对路径；未声明的临时文件不会发布给用户。
