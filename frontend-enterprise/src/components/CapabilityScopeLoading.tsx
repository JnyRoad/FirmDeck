...
/** Render a localized loading status for the employee capability surface. */
export default function CapabilityScopeLoading() {
  const { t } = useAppIntl();
  return (
    <div
      role="status"
      aria-label={t('capabilityScope.loading')}
    ...
    >
      ...
      <span className="sr-only">{t('capabilityScope.loading')}</span>
    </div>
  );
}
