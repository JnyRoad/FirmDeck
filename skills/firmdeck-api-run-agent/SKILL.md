---
name: firmdeck-api-run-agent
description: Run a FirmDeck digital employee through Open API v1, create or continue sessions, submit stateful or stateless runs, poll durable jobs, stream SSE events, continue awaiting-input SOPs, cancel work, and download Harness artifacts. Use whenever another agent must invoke a FirmDeck employee and return auditable results with citations and tool activity.
---

# Run a FirmDeck agent

## Prepare

1. Require `FIRMDECK_BASE_URL`, `FIRMDECK_API_KEY`, and an accessible `agent_id`.
