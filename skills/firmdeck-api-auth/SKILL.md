---
name: firmdeck-api-auth
description: Authenticate to FirmDeck Open API v1, inspect the current credential boundary, list accessible agents, browse the gallery, add a gallery agent, or bootstrap API clients and credentials. Use for FirmDeck API key setup, authorization failures, agent selection, scope checks, and cross-agent access questions.
---

# FirmDeck API authentication
...
1. Use `FIRMDECK_BASE_URL`; default to `http://39.102.210.77:10087/api/v1` only when the user has not supplied an environment.
2. Use `FIRMDECK_API_KEY` for `Authorization: Bearer ...`.
3. Fetch `$FIRMDECK_BASE_URL/openapi.json` before using an endpoint not listed in [references/endpoints.md](references/endpoints.md).
